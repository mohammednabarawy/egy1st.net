...
global $post;
$i = 0;
$args = array("post_type" => "post", "fields" => "ids", "meta_key" => "pin", "posts_per_page" => wp_is_mobile() ? 5 : 10, "meta_query" => array(array("key" => "number", "value" => "", "compare" => "=")));
...
if (isset($_GET["key"])) {
    if ($_GET["key"] == "latest") {
        $args = array("post_type" => "post", "fields" => "ids", "posts_per_page" => $postNumber, "meta_query" => array(array("key" => "number", "value" => "", "compare" => "=")));
        $wp_query = new WP_Query();
        $wp_query->query($args);
        while ($wp_query->have_posts()) {
            $wp_query->the_post();
        ...
        if ($_GET["key"] == "famous") {
            $args = array("post_type" => "post", "meta_key" => "views", "orderby" => "meta_value_num", "fields" => "ids", "posts_per_page" => $postNumber, "meta_query" => array(array("key" => "number", "value" => "", "compare" => "=")));
            $wp_query = new WP_Query();
            $wp_query->query($args);
            while ($wp_query->have_posts()) {
                $wp_query->the_post();
            ...
            $args = array("post_type" => "post", "meta_key" => $_GET["key"], "fields" => "ids", "posts_per_page" => $postNumber, "meta_query" => array(array("key" => "number", "value" => "", "compare" => "=")));
            if (isset($_GET["key"]) && $_GET["key"] == "imdbRating") {
