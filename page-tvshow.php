...
global $post;
$i = 0;
$args = array("post_type" => "post", "fields" => "ids", "meta_key" => "pin", "posts_per_page" => wp_is_mobile() ? 5 : 10, "meta_query" => array(array("key" => "TvPro", "compare" => "EXISTS")));
...
echo "<a href=\"" . home_url() . "/tvshow?key=new\">";
echo "<i class=\"far fa-video-plus\"></i><span>احدث الحلقات</span></a></li>";
echo "<li " . (isset($_GET["key"]) && $_GET["key"] == "latest" ? "class=\"active\"" : "") . ">";
echo "<a href=\"" . home_url() . "/tvshow?key=latest\">";
echo "<i class=\"fas fa-tv-retro\"></i><span>احدث البرامج</span></a></li>";
echo "<li " . (isset($_GET["key"]) && $_GET["key"] == "newSeries" ? "class=\"active\"" : "") . ">";
echo "<a href=\"" . home_url() . "/tvshow?key=newSeries\">";
echo "<i class=\"fal fa-plus\"></i><span>برامج جديدة</span></a></li>";
echo "<li " . (isset($_GET["key"]) && $_GET["key"] == "best" ? "class=\"active\"" : "") . ">";
echo "<a href=\"" . home_url() . "/tvshow?key=best\">";
echo "<i class=\"far fa-star\"></i><span>افضل البرامج</span></a></li>";
echo "<li " . (isset($_GET["key"]) && $_GET["key"] == "famous" ? "class=\"active\"" : "") . ">";
echo "<a href=\"" . home_url() . "/tvshow?key=famous\">";
echo "<i class=\"fas fa-eye\"></i><span>البرامج الاكثر شهرة</span></a></li></ul></div><hr class=\"slash-1\"></div><div class=\"MainRelated\" data-loading=\"false\">";
global $post;
...
if (isset($_GET["key"])) {
    if ($_GET["key"] == "new") {
        $args = array("post_type" => "post", "fields" => "ids", "posts_per_page" => $postNumber, "meta_key" => "TvPro");
        $wp_query = new WP_Query();
        $wp_query->query($args);
        while ($wp_query->have_posts()) {
            $wp_query->the_post();
    ...
    if ($_GET["key"] == "latest") {
        $series = array_slice(get_categories(array("taxonomy" => "tvshow")), 0, $postNumber);
    } else {
        $series = array_slice(get_categories(array("taxonomy" => "tvshow", "meta_key" => $_GET["key"])), 0, $postNumber);
    }
